module.exports = function(opts, definition) {
  ...
  return definition ? undefined : table
}
...
Table.prototype.end = function(width) {
  ...
  for (var i = 0; i < this.rows.length; ++i) {
    ...
    var height = row.end(columns, { table: { row: i }, doc: this.doc })
    ...
    if (height === false) {
      ...
      if (this._beforeBreak) {
        while (height === false && i > 0) {
          ...
          height = this._beforeBreak.end(columns, { table: { row: i }, doc: this.doc })
          if (height === false) {
            page.cursor.x = left
            page.cursor.y = y
      ...
      if (this._afterBreak) {
        page.cursor.x = left
        height += this._afterBreak.end(columns, context)
      ...
      if (this.opts.header === true) {
        page.cursor.x = left
        height += this.rows[0].end(columns, context)
      }
    } else {
      transaction.commit()
    }
...
Row.prototype.end = function(columns, context) {
  var page = this.doc.cursor
    , left = page.cursor.x
    , y = page.cursor.y
    , heights = []
  ...
  for (var i = 0, len = this.cells.length; i < len; ++i) {
    ...
    cell.end(innerWidth, context) // below: padding bottom
    var height = pageIndex + 1 < this.doc.pages.count
        ? y - this.doc.opts.padding.bottom
    ...
    for (var j = pageIndex + 1, count = this.doc.pages.count; j < count; ++j) {
      height += this.doc.height - this.doc.opts.padding.top - this.doc.pages.pages[j].cursor.y
    }
    heights.push(height)
    page.cursor.y = y
...
Cell.prototype.end = function(width, context) {
  this.content.end(width, context)
}

Cell.prototype.drawBorder = function(page, x, y, width, height, splitBelow, splitAbove) {
  if (y - this.doc.opts.padding.bottom < height) {
    var spaceLeft = height - y + this.doc.opts.padding.bottom
    var pageIndex = this.doc.pages.pages.indexOf(page)
    this.drawBorder(page, x, y, width, y - this.doc.opts.padding.bottom, true, false)
    while (spaceLeft > 0) {
      page = this.doc.pages.pages[++pageIndex]
      if (spaceLeft > this.doc.innerHeight) {
        height = this.doc.innerHeight
        this.drawBorder(page, this.doc.opts.padding.left, this.doc.height - this.doc.opts.padding.top, width, height, true, true)
      } else {
        height = spaceLeft // this.doc.height - this.doc.opts.padding.top - page.cursor.y
        this.drawBorder(page, this.doc.opts.padding.left, this.doc.height - this.doc.opts.padding.top, width, height, false, true)
      }
      spaceLeft -= height
    }
    return
  ...
  var downTo = y - height
  if (downTo < this.doc.opts.padding.bottom)
    downTo = this.doc.opts.padding.bottom
